...
@set CPPFLAGS=/DUSE_SSE4 /DUSE_MKL /DUSE_WINAPI /DDEBUG /DUSE_CLBLAST /Isrc\common /Iwin\include
...
cl %CPPFLAGS% %CXXFLAGS% src\autousi\autousi.cpp src\autousi\client.cpp src\autousi\pipe.cpp src\common\iobase.cpp src\common\option.cpp src\common\jqueue.cpp src\common\xzi.cpp src\common\err.cpp src\common\shogibase.cpp src\common\osi.cpp %LDFLAGS% Ws2_32.lib liblzma.lib
@if %ERRORLEVEL% neq 0 exit /b %ERRORLEVEL%

cl %CPPFLAGS% %CXXFLAGS% src\ocldevs\ocldevs.cpp src\common\err.cpp src\common\opencl.cpp objs\OpenCL.lib
@if %ERRORLEVEL% neq 0 exit /b %ERRORLEVEL%
